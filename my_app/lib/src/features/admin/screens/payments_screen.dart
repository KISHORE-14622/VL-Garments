...
class _PaymentsScreenState extends State<PaymentsScreen> {
  ...
  void _calculatePendingPayments() {
    ...
    for (var worker in widget.dataService.workers) {
      ...
      print('Worker ${worker.name} (${worker.id}): ${workerEntries.length} entries');
  ...
  Widget build(BuildContext context) {
    final totalPending = _staffPaymentData.values
        .fold<double>(0, (sum, data) => sum + (data['pendingAmount'] as double));
    final staffCount = _staffPaymentData.length;
      ...
      body: _loading
          ...
          : Column(
              children: [
                ...
                // Payment List
                Expanded(
                  child: _staffPaymentData.isEmpty
                      ? _buildEmptyState()
                      : ListView.builder(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          itemCount: _staffPaymentData.length,
                          itemBuilder: (context, index) {
                            final workerId = _staffPaymentData.keys.elementAt(index);
                            final data = _staffPaymentData[workerId]!;
